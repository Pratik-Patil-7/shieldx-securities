...
public class SecurityTypeService {
	...
	public List<SecurityTypeResponse> getAllSecurityTypess() {
		return securityTypeRepo.findAll().stream().map(st -> new SecurityTypeResponse(st.getStId(), st.getLevelName(),
				st.getDescription(), st.getIsArmed(), st.getPricePerDay())).collect(Collectors.toList());
	}
	...
	public List<String> getAllSecurityLevels() {
		return securityTypeRepo.findAll().stream().map(SecurityType::getLevelName).collect(Collectors.toList());
	}
	...
	public SecurityTypeResponse addSecurityType(SecurityTypeRequest request) {
		...
		return new SecurityTypeResponse(securityType.getStId(), securityType.getLevelName(),
				securityType.getDescription(), securityType.getIsArmed(), securityType.getPricePerDay());
	}
